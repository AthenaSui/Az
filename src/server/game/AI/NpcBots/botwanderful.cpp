...
std::string WanderNode::ToString(int32 link_weight/* = -1*/) const
{
    ...
    wps << "游荡点 " << _wpId << (link_weight >= 0 ? (":" + Bcore::ToString(link_weight)) : std::string{})
        << " '" << _name << "', " << uint32(_links.size()) << " 链接（平均权重 " << GetAverageLinkWeight()
        << "），地图 " << _mapId << ", 地区 " << _zoneId << ", 区域 " << _areaId << ", 最小等级 " << uint32(_minLevel) << ", 最大等级 " << uint32(_maxLevel)
