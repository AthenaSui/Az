...
Item* Item::CreateItem(uint32 item, uint32 count, Player const* player, bool clone, uint32 randomPropertyId, bool temp)//<-playerbot
{
    if (count < 1)
    ...
    if (pProto)
    {
        ...
        uint32 guid = temp ? 0xFFFFFFFF : sObjectMgr->GetGenerator<HighGuid::Item>().Generate();//<-playerbot
        if (pItem->Create(guid, item, player))//<-playerbot
        {
